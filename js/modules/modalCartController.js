...
const cartBtns = document.querySelectorAll(".modal-cart__btns");
...
const cartbBockElem = document.querySelector(".modal-cart__block");
  ...
  if (cartControl.cartData.length) {
    ...
    const cardsData = await Promise.all(
      cartControl.cartData.map(
        async (item) =>
          await getData(
            `https://go-go-pizza-api-14eo.onrender.com/api/products/${item.id}`
          )
      )
    );
    ...
    cartListElem.innerHTML = "<li>Кошик порожній...</li>";
    nextBtn.disabled = true;
  }

  cartPriceElem.forEach((elem) => (elem.textContent = `${totalPrice} грн`));
};
...
const changeNextBlock = () => {
  cartbBockElem.style.display = "none";
  cartDeliveryElem.style.display = "block";
};

const changePrevBlock = () => {
  cartbBockElem.style.display = "block";
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch(
      "https://go-go-pizza-api-14eo.onrender.com/api/orders/",
      {
        method: "POST",
        headers: {
    ...
    cartForm.innerHTML = `
        <h3>Замовлення оформлено, номер замовлення ${order.orderId}</h3>
        `;
